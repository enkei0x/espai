...
OpenAICompatibleProvider::OpenAICompatibleProvider(const OpenAICompatibleConfig& config)
    : _config(config)
    , _name(config.name)
{
...
bool OpenAICompatibleProvider::isConfigured() const {
    if (_config.requiresApiKey && _apiKey.length() == 0) {
        return false;
    }
    return _model.length() > 0;
}

void OpenAICompatibleProvider::addCustomHeader(const String& name, const String& value) {
    _customHeaders.push_back({name, value});
}

void OpenAICompatibleProvider::addAuthHeaders(HttpRequest& req) {
    if (_config.authHeaderName.isEmpty()) {
        return;
    }
    if (_apiKey.isEmpty()) {
        return;
    }
    addHeader(req, _config.authHeaderName, _config.authHeaderValuePrefix + _apiKey);
}

void OpenAICompatibleProvider::addProviderHeaders(HttpRequest& req) {
    (void)req;
}

HttpRequest OpenAICompatibleProvider::buildHttpRequest(
    ...
    addAuthHeaders(req);
    addProviderHeaders(req);

    for (const auto& header : _customHeaders) {
        addHeader(req, header.first, header.second);
    }

    return req;
}

String OpenAICompatibleProvider::buildRequestBody(
    const std::vector<Message>& messages,
    const ChatOptions& options
) {
    JsonDocument doc;
...
#if ESPAI_ENABLE_TOOLS
    if (_config.toolCallingSupported && !_tools.empty()) {
...
Response OpenAICompatibleProvider::parseResponse(const String& json) {
    Response response;
...
#if ESPAI_ENABLE_TOOLS
Message OpenAICompatibleProvider::getAssistantMessageWithToolCalls(const String& content) const {
